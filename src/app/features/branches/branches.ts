...
export class Branches {
  ...
  protected readonly result = signal<Page<Branch> | null>(null);
  ...
  protected submit(): void {
    ...
    this.formError.set(null);
    this.globalLoading.start('Guardando delegación', 'Registrando la nueva delegación.');
    ...
    this.service
      ...
      .subscribe({
        next: () => {
          ...
          this.closeModal();
          this.notify.success('Delegación creada');
          this.reload(0);
        },
        error: (err: HttpErrorResponse) => {
          this.submitting.set(false);
...
function extractMessage(error: HttpErrorResponse): string {
  ...
  return error.message || 'Error al cargar las delegaciones';
}
